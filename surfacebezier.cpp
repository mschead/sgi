...
SurfaceBezier::SurfaceBezier(const char* nome, vector<Point*> coordenadas,
...
void SurfaceBezier::clipping(Window window, Viewport viewport) {
    // método de clippagem da reta invocado
}

void SurfaceBezier::draw(Viewport viewport, Window window, cairo_t *cr, int clippingType) {
    auxLines.clear();
    ...
    for (int i = 0; i < auxLines.size(); i++) {
        auxLines.at(i)->draw(viewport, window, cr, clippingType);
    }
}
