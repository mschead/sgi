...
public:
	Object3D(char* nome, vector<Line*> lines) {
		this->nome = nome;
		this->lines = lines;
		...
		for( int i = 2; i < lines.size(); i++){
			...
			bool adicionarCoordenada2 = true;
				...
				for( int j = 0; j<coordenadas.size(); j++){
					if ( coordenada1->getX() == coordenadas.at(j)->getX() && coordenada1->getY() == coordenadas.at(j)->getY()){
						//marca para nao adicionar a coordenada1
						adicionarCoordenada1 = false;
					}
					if ( coordenada2->getX() == coordenadas.at(j)->getX() && coordenada2->getY() == coordenadas.at(j)->getY()){
